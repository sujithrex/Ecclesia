...
const AddCustomBookCreditPage = ({
  ...
  const loadCategories = async () => {
    try {
      ...
      const result = await categoryAPI.getWithSubcategoriesByType({ ...params, categoryType: 'income' });
  ...
  const loadTransactionData = async () => {
    try {
      const result = isChurchLevel
        ? await window.electron.churchCustomBookTransaction.getCreditTransaction({ transactionId: parseInt(transactionId) })
        : await window.electron.customBookTransaction.getCreditTransaction({ transactionId: parseInt(transactionId) });
  ...
  const handleSubmit = async (closeAfter = false) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        const result = isChurchLevel
          ? await window.electron.churchCustomBookTransaction.updateCreditTransaction({
              transactionId: parseInt(transactionId),
              transactionData,
              userId: parseInt(userId),
            })
          : await window.electron.customBookTransaction.updateCreditTransaction({
              transactionId: parseInt(transactionId),
              transactionData,
              userId: parseInt(userId),
            });
        ...
        if (result.success) {
          showAlert('Credit voucher updated successfully!', 'success');
          setTimeout(() => {
            navigate(isChurchLevel ? `/church-custom-book-credit/${bookId}?church=true` : `/custom-book-credit/${bookId}`);
          }, 1500);
        } else {
          showAlert(result.error || 'Failed to update credit voucher', 'error');
        }
      } else {
        ...
        const result = isChurchLevel
          ? await window.electron.churchCustomBookTransaction.createCredit({
              transactionData,
              userId: parseInt(userId),
            })
          : await window.electron.customBookTransaction.createCredit({
              transactionData,
              userId: parseInt(userId),
            });

        if (result.success) {
          showAlert('Credit voucher created successfully!', 'success');
          ...
          showAlert(result.error || 'Failed to create credit voucher', 'error');
        }
      }
    } catch (error) {
      console.error('Error saving credit voucher:', error);
  ...
  return (
    <div className={styles.container}>
      <Breadcrumb
        pageTitle={`Credit Voucher - ${book?.book_name || 'Loading...'}`}
      ...
      <div className={styles.content}>
        <div className={styles.formCard}>
          <h2 className={styles.formTitle}>
            <MoneyRegular />
            Credit Voucher - {book?.book_name || 'Loading...'}
          </h2>
...
export default AddCustomBookCreditPage;
