...
const AddOtherCreditTransactionPage = ({
  user,
  onLogout,
  onProfileClick,
  currentPastorate,
  ...
  const generateTransactionId = async () => {
    try {
      const result = await window.electron.otherCredits.generateTransactionId();
      if (result.success) {
        setFormData(prev => ({
          ...prev,
  ...
  const loadNextCreditNumber = async () => {
    try {
      const result = await window.electron.otherCredits.getNextCreditNumber({
  ...
  const loadTransaction = async () => {
    try {
      const result = await window.electron.otherCredits.getTransaction({
        transactionId
      });
  ...
  const searchFamilies = async (searchTerm) => {
    try {
      const result = await window.electron.otherCredits.searchFamilies({
        pastorateId: currentPastorate.id,
        searchTerm
      });
  ...
  const handleSubmit = async (closeAfter = false) => {
    ...
    try {
      const result = isEditMode
        ? await window.electron.otherCredits.updateTransaction({
            id: transactionId,
            ...formData,
        ...
        : await window.electron.otherCredits.createTransaction({
            ...formData,
            credit_number: parseInt(formData.credit_number),
            primary_category_id: formData.primary_category_id || null,
            secondary_category_id: formData.secondary_category_id || null,
      ...
      if (result.success) {
        if (closeAfter) {
          ...
          navigate(`/other-credits?bookType=${bookType}`);
        } else {
  ...
  return (
    <div className={styles.container}>
      ...
      <Breadcrumb
        pageTitle={`${isEditMode ? 'Edit' : 'Add'} Other Credit Transaction (${bookType.charAt(0).toUpperCase() + bookType.slice(1)} Book) - ${currentPastorate.pastorate_name}`}
        titleAlign="left"
        breadcrumbs={[
          {
          ...
          },
          {
            ...
            onClick: () => navigate(`/other-credits?bookType=${bookType}`)
          },
          {
...
export default AddOtherCreditTransactionPage;