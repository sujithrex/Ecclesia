...
const AddBillVoucherTransactionPage = ({
  user,
  onLogout,
  onProfileClick,
  currentPastorate,
  ...
  const loadTransaction = async () => {
    setLoading(true);
    try {
      const result = await window.electron.billVoucher.getTransaction({
  ...
  const generateTransactionId = async () => {
    try {
      const result = await window.electron.billVoucher.generateTransactionId();
      if (result.success) {
        setFormData(prev => ({
          ...prev,
  ...
  const loadNextVoucherNumber = async () => {
    try {
      const result = await window.electron.billVoucher.getNextVoucherNumber({
  ...
  const handleSubmit = async (closeAfter = true) => {
    ...
    try {
      const result = isEditMode
        ? await window.electron.billVoucher.updateTransaction({
            id: transactionId,
            ...formData,
        ...
        : await window.electron.billVoucher.createTransaction({
            ...formData,
            voucher_number: parseInt(formData.voucher_number),
            primary_category_id: formData.primary_category_id || null,
            secondary_category_id: formData.secondary_category_id || null,
      ...
      if (result.success) {
        if (closeAfter) {
          ...
          navigate(`/bill-vouchers?bookType=${bookType}`);
        } else {
      ...
      <Breadcrumb
        ...
        breadcrumbs={[
          ...
          },
          {
            ...
            onClick: () => navigate(`/bill-vouchers?bookType=${bookType}`)
          },
          {
...
export default AddBillVoucherTransactionPage;
