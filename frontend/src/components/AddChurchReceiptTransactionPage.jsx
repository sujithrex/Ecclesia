...
const AddChurchReceiptTransactionPage = ({
  user,
  onLogout,
  onProfileClick,
  currentPastorate,
  ...
  const generateTransactionId = async () => {
    try {
      const result = await window.electron.churchReceipts.generateTransactionId();
      if (result.success) {
        setFormData(prev => ({
          ...prev,
  ...
  const loadNextReceiptNumber = async () => {
    try {
      const result = await window.electron.churchReceipts.getNextReceiptNumber({
  ...
  const loadTransaction = async () => {
    try {
      const result = await window.electron.churchReceipts.getTransaction({
        transactionId
      });
  ...
  const searchFamilies = async (searchTerm) => {
    try {
      const result = await window.electron.churchReceipts.searchFamilies({
        churchId: currentChurch.id,
        searchTerm
      });
  ...
  const handleSubmit = async (closeAfter = false) => {
    ...
    try {
      const result = isEditMode
        ? await window.electron.churchReceipts.updateTransaction({
            id: transactionId,
            ...formData,
        ...
        : await window.electron.churchReceipts.createTransaction({
            ...formData,
            receipt_number: parseInt(formData.receipt_number),
            churchId: currentChurch.id,
            bookType: bookType,
      ...
      if (result.success) {
        if (closeAfter) {
          ...
          navigate(`/church-receipts?bookType=${bookType}`);
        } else {
  ...
  return (
    <div className={styles.container}>
      ...
      <Breadcrumb
        pageTitle={`${isEditMode ? 'Edit' : 'Add'} Receipt Transaction (${bookType.charAt(0).toUpperCase() + bookType.slice(1)} Book) - ${currentChurch.church_name}`}
        titleAlign="left"
        breadcrumbs={[
          {
          ...
          },
          {
            ...
            onClick: () => navigate(`/church-receipts?bookType=${bookType}`)
          },
          {
...
export default AddChurchReceiptTransactionPage;
