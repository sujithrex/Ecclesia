...
const ChurchContraVouchersPage = ({
  user,
  onLogout,
  onProfileClick,
  currentPastorate,
  ...
  }, [currentChurch?.id, user?.id, bookType]);
  ...
  const loadAllTransactions = async () => {
    setLoading(true);
    try {
      const result = await window.electron.churchContra.getTransactions({
  ...
  const handleAddTransaction = () => {
    navigate(`/church-contra-vouchers/add?bookType=${bookType}`);
  };

  const handleEditTransaction = (transaction) => {
    navigate(`/church-contra-vouchers/edit/${transaction.id}?bookType=${bookType}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const result = await window.electron.churchContra.deleteTransaction({
...
export default ChurchContraVouchersPage;
