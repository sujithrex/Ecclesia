...
const AddAcquittanceTransactionPage = ({
  user,
  onLogout,
  onProfileClick,
  currentPastorate,
  ...
  const searchPayees = async (searchTerm) => {
    try {
      const result = await window.electron.acquittance.getPayeesByCategory({
  ...
  const loadLastAmount = async (payeeName) => {
    try {
      const result = await window.electron.acquittance.getLastAmountForPayee({
        pastorateId: currentPastorate.id,
        bookType: bookType,
        primaryCategoryId: formData.primary_category_id,
        secondaryCategoryId: formData.secondary_category_id,
  ...
  const loadTransaction = async () => {
    setLoading(true);
    try {
      const result = await window.electron.acquittance.getTransaction({
  ...
  const generateTransactionId = async () => {
    try {
      const result = await window.electron.acquittance.generateTransactionId();
      if (result.success) {
        setFormData(prev => ({
          ...prev,
  ...
  const loadNextVoucherNumber = async () => {
    try {
      const result = await window.electron.acquittance.getNextVoucherNumber({
        pastorateId: currentPastorate.id,
        bookType: bookType
      });
  ...
  const handleSubmit = async (closeAfter = true) => {
    ...
    try {
      const result = isEditMode
        ? await window.electron.acquittance.updateTransaction({
            id: transactionId,
            ...formData,
        ...
        : await window.electron.acquittance.createTransaction({
            ...formData,
            voucher_number: parseInt(formData.voucher_number),
            primary_category_id: formData.primary_category_id || null,
            secondary_category_id: formData.secondary_category_id || null,
      ...
      if (result.success) {
        if (closeAfter) {
          ...
          navigate(`/acquittance?bookType=${bookType}`);
        } else {
      ...
      <Breadcrumb
        ...
        breadcrumbs={[
          ...
          },
          {
            ...
            onClick: () => navigate(`/acquittance?bookType=${bookType}`)
          },
          {
...
export default AddAcquittanceTransactionPage;
