...
const ChurchOtherCreditsPage = ({
  ...
  const loadTransactions = async () => {
    try {
      setLoading(true);
      const result = await window.electron.churchOtherCredits.getTransactions({
  ...
  const handleAddTransaction = () => {
    navigate(`/church-other-credits/add?bookType=${bookType}`);
  };

  const handleEditTransaction = (transaction) => {
    navigate(`/church-other-credits/edit/${transaction.id}?bookType=${bookType}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const result = await window.electron.churchOtherCredits.deleteTransaction({
  ...
  return (
    <div className={styles.container}>
      ...
      <Breadcrumb
        pageTitle={`Other Credits - ${currentChurch.church_name}`}
        titleAlign="left"
        breadcrumbs={[
          {
...
export default ChurchOtherCreditsPage;
