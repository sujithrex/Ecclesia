...
const CustomBookDebitPage = ({
  ...
  const handleAddTransaction = () => {
    navigate(isChurchLevel ? `/church-custom-book-debit/${bookId}/add?church=true` : `/custom-book-debit/${bookId}/add`);
  };

  const handleEditTransaction = (transaction) => {
    navigate(isChurchLevel ? `/church-custom-book-debit/${bookId}/edit/${transaction.id}?church=true` : `/custom-book-debit/${bookId}/edit/${transaction.id}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const userId = localStorage.getItem('userId');
      const result = isChurchLevel
        ? await window.electron.churchCustomBookTransaction.deleteDebitTransaction({
            transactionId: transactionToDelete.id,
            userId: parseInt(userId),
          })
        : await window.electron.customBookTransaction.deleteDebitTransaction({
            transactionId: transactionToDelete.id,
            userId: parseInt(userId),
          });
  ...
  return (
    <div className={styles.container}>
      <Breadcrumb
        pageTitle={`Debit Vouchers - ${book?.book_name || 'Loading...'}`}
          ...
          <div className={styles.tableWrapper}>
            {loading ? (
              ...
              <div className={styles.emptyState}>
                ...
                <div className={styles.emptyStateText}>No debit vouchers found</div>
                <div className={styles.emptyStateSubtext}>Click "Add Transaction" to create your first debit voucher</div>
              </div>
            ) : (
...
export default CustomBookDebitPage;
