...
const AddChurchAcquittanceTransactionPage = ({
  user,
  onLogout,
  onProfileClick,
  currentPastorate,
  ...
  // Clear payee and amount when secondary category changes
  useEffect(() => {
    if (!isEditMode) {
      setFormData(prev => ({
        ...prev,
  ...
  // Debounced payee search when user types
  useEffect(() => {
    if (searchTimeout) {
      clearTimeout(searchTimeout);
    }

    // Only search if we have both categories selected and payee name has at least 2 characters
    if (formData.primary_category_id && formData.secondary_category_id && formData.payee_name && formData.payee_name.length >= 2) {
      const timeout = setTimeout(() => {
        searchPayees(formData.payee_name);
      }, 300);
  ...
  }, [formData.payee_name, formData.primary_category_id, formData.secondary_category_id]);
  ...
  const searchPayees = async (searchTerm) => {
    try {
      const result = await window.electron.churchAcquittance.getPayeesByCategory({
        churchId: currentChurch.id,
        bookType: bookType,
        primaryCategoryId: formData.primary_category_id,
        secondaryCategoryId: formData.secondary_category_id,
        searchTerm: searchTerm
      });

      if (result.success && isMountedRef.current) {
        setPayeeSuggestions(result.payees || []);
      }
    } catch (error) {
  ...
  const loadLastAmount = async (payeeName) => {
    try {
      const result = await window.electron.churchAcquittance.getLastAmountForPayee({
        churchId: currentChurch.id,
        bookType: bookType,
        primaryCategoryId: formData.primary_category_id,
        secondaryCategoryId: formData.secondary_category_id,
  ...
  const handlePayeeSelect = (payee) => {
    ...
    setPayeeSuggestions([]);
    // Load last amount for this payee
    loadLastAmount(payee.payee_name);
  };

  const loadTransaction = async () => {
    setLoading(true);
    try {
      const result = await window.electron.churchAcquittance.getTransaction({
  ...
  const generateTransactionId = async () => {
    try {
      const result = await window.electron.churchAcquittance.generateTransactionId();
      if (result.success) {
        setFormData(prev => ({
          ...prev,
  ...
  const loadNextVoucherNumber = async () => {
    try {
      const result = await window.electron.churchAcquittance.getNextVoucherNumber({
        churchId: currentChurch.id,
        bookType: bookType
      });
  ...
  const loadPrimaryCategories = async () => {
    try {
      ...
      if (result.success) {
        // Filter only expense categories for acquittance
  ...
  const validateForm = () => {
    ...
    } else if (!formData.transaction_id.match(/^AQ-[A-Z0-9]{5}$/)) {
      newErrors.transaction_id = 'Transaction ID must be in format AQ-XXXXX';
    }
  ...
  const handleSubmit = async (closeAfter = true) => {
    ...
    try {
      const result = isEditMode
        ? await window.electron.churchAcquittance.updateTransaction({
            id: transactionId,
            ...formData,
        ...
        : await window.electron.churchAcquittance.createTransaction({
            ...formData,
            voucher_number: parseInt(formData.voucher_number),
            primary_category_id: formData.primary_category_id || null,
            secondary_category_id: formData.secondary_category_id || null,
      ...
      if (result.success) {
        if (closeAfter) {
          ...
          navigate(`/church-acquittance?bookType=${bookType}`);
        } else {
          // Show notification for "Save & Create Another"
          if (isMountedRef.current) {
            setNotification({
              type: 'success',
              message: `Acquittance transaction ${isEditMode ? 'updated' : 'created'} successfully!`
            });
  ...
  return (
    <div className={styles.container}>
      ...
      <Breadcrumb
        pageTitle={`${isEditMode ? 'Edit' : 'Add'} Acquittance - ${getBookTypeName()}`}
        titleAlign="left"
        breadcrumbs={[
          {
          ...
          },
          {
            ...
            onClick: () => navigate(`/church-acquittance?bookType=${bookType}`)
          },
          {
      ...
      <div className={styles.content}>
        ...
        <div className={styles.formCard}>
          <h2 className={styles.formTitle}>
            {isEditMode ? 'Edit Acquittance Transaction' : 'Add Acquittance Transaction'}
          </h2>
          ...
          <div className={styles.formGrid}>
            <div className={styles.formGroup}>
              ...
              <input
                ...
                maxLength={8}
                disabled={loading}
                placeholder="e.g., AQ-A1B2C"
              />
              {errors.transaction_id && (
                <span className={styles.errorMessage}>{errors.transaction_id}</span>
              )}
              {!errors.transaction_id && (
                <span className={styles.helperText}>Format: AQ-XXXXX (5 alphanumeric characters)</span>
              )}
            </div>
            ...
            <div className={`${styles.formGroup} ${styles.formGroupFull}`}>
              ...
              <div className={styles.payeeDropdown} ref={dropdownRef}>
                <input
                  type="text"
                  className={`${styles.payeeInput} ${errors.payee_name ? styles.errorInput : ''}`}
                  value={formData.payee_name}
                  onChange={(e) => handleInputChange('payee_name', e.target.value)}
                  placeholder={
                    formData.primary_category_id && formData.secondary_category_id
                      ? "Type payee name (search previous payees)"
                      : "Select category and subcategory first"
                  }
                  disabled={loading || !formData.primary_category_id || !formData.secondary_category_id}
                  autoComplete="off"
                />
                {payeeSuggestions.length > 0 && (
                  <div className={styles.payeeDropdownList}>
                    {payeeSuggestions.map((payee, index) => (
                      <div
                        key={index}
                        className={styles.payeeDropdownItem}
              ...
              {!errors.payee_name && (
                <span className={styles.helperText}>
                  {formData.primary_category_id && formData.secondary_category_id
                    ? 'Type at least 2 characters to search previous payees in this category, or enter a new name'
                    : 'Select both category and subcategory first to enable payee search'}
                </span>
              )}
            </div>
...
export default AddChurchAcquittanceTransactionPage;
