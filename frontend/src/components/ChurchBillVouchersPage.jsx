...
const ChurchBillVouchersPage = ({
  ...
  }, [currentChurch?.id, user?.id, currentPage, selectedMonth, bookType]);
  ...
  const loadTransactions = async () => {
    setLoading(true);
    try {
      const result = await window.electron.churchBillVoucher.getTransactions({
  ...
  const loadStatistics = async () => {
    try {
      const result = await window.electron.churchBillVoucher.getStatistics({
        churchId: currentChurch.id,
        bookType: bookType
      });
  ...
  const handleAddTransaction = () => {
    navigate(`/add-bill-voucher?bookType=${bookType}`);
  };

  const handleEditTransaction = (transactionId) => {
    navigate(`/add-bill-voucher/${transactionId}?bookType=${bookType}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const result = await window.electron.churchBillVoucher.deleteTransaction({
      ...
      if (result.success) {
        setNotification({
          type: 'success',
          message: 'Bill voucher transaction deleted successfully!'
        });
        loadTransactions();
        loadStatistics();
      } else {
  ...
  return (
    <div className={styles.container}>
      ...
      <Breadcrumb
        pageTitle={`Bills / Vouchers - ${getBookTypeName()}`}
        titleAlign="left"
        breadcrumbs={[
          {
          ...
          <div className={styles.tableWrapper}>
            {loading ? (
              ...
              <div className={styles.emptyState}>
                ...
                <div className={styles.emptyStateText}>No bill voucher transactions found</div>
                <div style={{ fontSize: '14px', color: '#8a8886' }}>
                  Click "Add Transaction" to create your first bill voucher
                </div>
              </div>
            ) : (
        ...
        <div className={styles.deleteModal} onClick={handleDeleteCancel}>
          <div className={styles.deleteModalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <p className={styles.deleteModalText}>
              Are you sure you want to delete this bill voucher transaction?
              <br />
...
export default ChurchBillVouchersPage;
