class ChurchContraService {
  ...
  async generateTransactionId() {
    return new Promise((resolve) => {
      ...
      const checkAndGenerate = () => {
        const id = generateId();
        this.db.db.get(
          'SELECT id FROM church_contra_transactions WHERE transaction_id = ?',
          [id],
          (err, existing) => {
            if (err) {
  ...
  async getNextVoucherNumber(churchId, bookType) {
    return new Promise((resolve) => {
      try {
        this.db.db.get(
          'SELECT MAX(voucher_number) as max_number FROM church_contra_transactions WHERE church_id = ? AND book_type = ?',
          [churchId, bookType],
          (err, result) => {
            if (err) {
  ...
  async createTransaction(transactionData, userId) {
    return new Promise((resolve) => {
      try {
        ...
        if (!['church', 'pastorate'].includes(from_account_level)) {
          resolve({
            success: false,
            error: 'Invalid from account level. Must be church or pastorate'
          });
          return;
        }

        if (!['church', 'pastorate'].includes(to_account_level)) {
          resolve({
            success: false,
            error: 'Invalid to account level. Must be church or pastorate'
          });
          return;
        }
        ...
        this.db.db.get(
          ...
          (err, existing) => {
            ...
            this.db.db.get(
              'SELECT id FROM church_contra_transactions WHERE church_id = ? AND book_type = ? AND voucher_number = ?',
  ...
  async getTransactions(churchId, userId, bookType, page = 1, limit = 10, filters = {}) {
    return new Promise((resolve) => {
      try {
        ...
        let whereClause = 'WHERE ct.church_id = ? AND ct.book_type = ?';
        ...
        this.db.db.all(
          ...
          FROM church_contra_transactions ct
          LEFT JOIN church_ledger_categories fc ON ct.from_category_id = fc.id
          LEFT JOIN church_ledger_categories tc ON ct.to_category_id = tc.id
          ${whereClause}
          ...
          (err, transactions) => {
            ...
            // Get total count
            this.db.db.get(
              `SELECT COUNT(*) as total FROM church_contra_transactions ct ${whereClause}`,
              queryParams,
              (err, countResult) => {
                if (err) {
  ...
  async updateTransaction(transactionId, transactionData, userId) {
    return new Promise((resolve) => {
      try {
        ...
        this.db.db.get(
          'SELECT id FROM church_contra_transactions WHERE transaction_id = ? AND id != ?',
          ...
          (err, existing) => {
            ...
            this.db.db.get(
              'SELECT id FROM church_contra_transactions WHERE church_id = ? AND book_type = ? AND voucher_number = ? AND id != ?',
  ...
  async deleteTransaction(transactionId, userId) {
    return new Promise((resolve) => {
      try {
        this.db.db.run(
          'DELETE FROM church_contra_transactions WHERE id = ?',
          [transactionId],
          function(err) {
            if (err) {
...
module.exports = ChurchContraService;
