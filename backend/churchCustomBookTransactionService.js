...
class ChurchCustomBookTransactionService {
  ...
  async createContraTransaction(transactionData, userId) {
    const { transactionId, voucherNumber, customBookId, churchId, fromName, toName, date, amount, notes } = transactionData;

    return new Promise((resolve) => {
      try {
        this.db.db.run(
          `INSERT INTO church_custom_book_contra_transactions
           (transaction_id, voucher_number, custom_book_id, church_id, from_name, to_name, date, amount, notes, created_by, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
          [transactionId, voucherNumber, customBookId, churchId, fromName, toName, date, amount, notes || null, userId],
          function(err) {
            if (err) {
