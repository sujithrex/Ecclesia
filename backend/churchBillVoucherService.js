class ChurchBillVoucherService {
  constructor(db) {
    this.db = db;
  }
  ...
  async generateTransactionId() {
    return new Promise((resolve) => {
      ...
      const checkAndGenerate = () => {
        const id = generateId();
        this.db.db.get(
          'SELECT id FROM church_bill_voucher_transactions WHERE transaction_id = ?',
          [id],
          (err, existing) => {
            if (err) {
      ...
      try {
        ...
        this.db.db.get(
          ...
          (err, existing) => {
            ...
            this.db.db.get(
              'SELECT id FROM church_bill_voucher_transactions WHERE church_id = ? AND book_type = ? AND voucher_number = ?',
              ...
              (err, existingVoucher) => {
                ...
                dbInstance.run(
                  `INSERT INTO church_bill_voucher_transactions
  ...
  async getTransactions(churchId, userId, bookType, page = 1, limit = 10, filters = {}) {
    return new Promise((resolve) => {
      try {
        ...
        let whereClause = 'WHERE bvt.church_id = ? AND bvt.book_type = ?';
        ...
        // Get total count
        this.db.db.get(
          `SELECT COUNT(*) as total FROM church_bill_voucher_transactions bvt ${whereClause}`,
          queryParams,
          (err, countResult) => {
            if (err) {
            ...
            this.db.db.all(
              ...
              FROM church_bill_voucher_transactions bvt
              LEFT JOIN church_ledger_categories lc ON bvt.primary_category_id = lc.id
              LEFT JOIN church_ledger_sub_categories lsc ON bvt.secondary_category_id = lsc.id
              ${whereClause}
  ...
  async updateTransaction(transactionId, transactionData, userId) {
    return new Promise((resolve) => {
      try {
        ...
        this.db.db.get(
          'SELECT id FROM church_bill_voucher_transactions WHERE transaction_id = ? AND id != ?',
          ...
          (err, existing) => {
            ...
            this.db.db.get(
              'SELECT id FROM church_bill_voucher_transactions WHERE church_id = ? AND book_type = ? AND voucher_number = ? AND id != ?',
              ...
              (err, existingVoucher) => {
                ...
                this.db.db.run(
                  `UPDATE church_bill_voucher_transactions
  ...
  async deleteTransaction(transactionId, userId) {
    return new Promise((resolve) => {
      try {
        this.db.db.run(
          'DELETE FROM church_bill_voucher_transactions WHERE id = ?',
          [transactionId],
          function(err) {
            if (err) {
  ...
  async getNextVoucherNumber(churchId, bookType) {
    return new Promise((resolve) => {
      try {
        this.db.db.get(
          'SELECT MAX(voucher_number) as max_number FROM church_bill_voucher_transactions WHERE church_id = ? AND book_type = ?',
          [churchId, bookType],
          (err, result) => {
            if (err) {
      ...
      try {
        ...
        this.db.db.get(
          `SELECT COALESCE(SUM(amount), 0) as total
           FROM church_bill_voucher_transactions
          ...
          (err, totalResult) => {
            ...
            this.db.db.all(
              ...
              FROM church_bill_voucher_transactions bvt
              LEFT JOIN church_ledger_categories lc ON bvt.primary_category_id = lc.id
              WHERE bvt.church_id = ? AND bvt.book_type = ?
...
module.exports = ChurchBillVoucherService;
