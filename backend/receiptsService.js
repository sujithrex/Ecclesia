class ReceiptsService {
  ...
  async generateTransactionId() {
    return new Promise((resolve) => {
      ...
      const checkAndGenerate = () => {
        const id = generateId();
        this.db.db.get(
          'SELECT id FROM receipt_transactions WHERE transaction_id = ?',
          [id],
          (err, existing) => {
            if (err) {
  ...
  async createTransaction(transactionData, userId) {
    return new Promise((resolve) => {
      try {
        ...
        if (!transaction_id || !receipt_number || !giver_name || !offering_type || !date || !amount || !pastorateId || !bookType) {
        ...
        this.db.db.get(
          ...
          (err, existingTransaction) => {
            ...
            this.db.db.get(
              'SELECT id FROM receipt_transactions WHERE pastorate_id = ? AND book_type = ? AND receipt_number = ?',
              [pastorateId, bookType, receipt_number],
              (err, existingReceipt) => {
                if (err) {
                ...
                dbInstance.run(
                  ...
                  (transaction_id, receipt_number, pastorate_id, book_type, family_id, giver_name, offering_type, date, amount, created_by, created_at, updated_at)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
                  [transaction_id, receipt_number, pastorateId, bookType, family_id || null, giver_name, offering_type, date, amount, userId],
                  function(err) {
                    if (err) {
  ...
  async getTransactions(pastorateId, userId, page = 1, limit = 10, filters = {}) {
    return new Promise((resolve) => {
      try {
        ...
        let whereClause = 'WHERE rt.pastorate_id = ?';
        ...
        this.db.db.all(
          ...
          (err, transactions) => {
            ...
            const countQuery = `SELECT COUNT(*) as count FROM receipt_transactions rt ${whereClause}`;
            this.db.db.get(
              countQuery,
              queryParams,
              (err, countResult) => {
  ...
  async updateTransaction(transactionId, transactionData, userId) {
    return new Promise((resolve) => {
      try {
        ...
        this.db.db.get(
          'SELECT id FROM receipt_transactions WHERE transaction_id = ? AND id != ?',
          ...
          (err, existingTransaction) => {
            ...
            this.db.db.get(
              'SELECT id FROM receipt_transactions WHERE pastorate_id = ? AND book_type = ? AND receipt_number = ? AND id != ?',
              [pastorateId, bookType, receipt_number, transactionId],
              (err, existingReceipt) => {
                if (err) {
                  console.error('Error checking receipt_number:', err);
                  resolve({
  ...
  async deleteTransaction(transactionId, userId) {
    return new Promise((resolve) => {
      try {
        this.db.db.run(
          'DELETE FROM receipt_transactions WHERE id = ?',
          [transactionId],
          function(err) {
            if (err) {
   ...
   */
  async getNextReceiptNumber(pastorateId, bookType) {
    return new Promise((resolve) => {
      try {
        this.db.db.get(
          'SELECT MAX(receipt_number) as max_number FROM receipt_transactions WHERE pastorate_id = ? AND book_type = ?',
   ...
   */
  async getStatistics(pastorateId) {
    return new Promise((resolve) => {
      try {
        // Get total amount
        this.db.db.get(
