class AcquittanceService {
  ...
  async generateTransactionId() {
    return new Promise((resolve) => {
      ...
      const checkAndGenerate = () => {
        const id = generateId();
        this.db.db.get(
          'SELECT id FROM acquittance_transactions WHERE transaction_id = ?',
          [id],
          (err, existing) => {
            if (err) {
   ...
   */
  async getPayeesByCategory(pastorateId, bookType, primaryCategoryId, secondaryCategoryId, searchTerm = '') {
    return new Promise((resolve) => {
      try {
        let whereClause = 'WHERE pastorate_id = ? AND book_type = ?';
   ...
   */
  async getLastAmountForPayee(pastorateId, bookType, primaryCategoryId, secondaryCategoryId, payeeName) {
    return new Promise((resolve) => {
      try {
        let whereClause = 'WHERE pastorate_id = ? AND book_type = ? AND payee_name = ?';
        let queryParams = [pastorateId, bookType, payeeName];
      ...
      try {
        ...
        this.db.db.get(
          ...
          (err, existing) => {
            ...
            this.db.db.get(
              'SELECT id FROM acquittance_transactions WHERE pastorate_id = ? AND book_type = ? AND voucher_number = ?',
              ...
              (err, existingVoucher) => {
                ...
                dbInstance.run(
                  `INSERT INTO acquittance_transactions
  ...
  async getTransactions(pastorateId, userId, bookType, page = 1, limit = 10, filters = {}) {
    return new Promise((resolve) => {
      try {
        ...
        let whereClause = 'WHERE aqt.pastorate_id = ? AND aqt.book_type = ?';
        let queryParams = [pastorateId, bookType];
        ...
        // Get total count
        this.db.db.get(
          `SELECT COUNT(*) as total FROM acquittance_transactions aqt ${whereClause}`,
          queryParams,
          (err, countResult) => {
            if (err) {
            ...
            this.db.db.all(
              ...
              LEFT JOIN ledger_categories lc ON aqt.primary_category_id = lc.id
              LEFT JOIN ledger_sub_categories lsc ON aqt.secondary_category_id = lsc.id
              ${whereClause}
  ...
  async updateTransaction(transactionId, transactionData, userId) {
    return new Promise((resolve) => {
      try {
        ...
        this.db.db.get(
          'SELECT id FROM acquittance_transactions WHERE transaction_id = ? AND id != ?',
          ...
          (err, existing) => {
            ...
            this.db.db.get(
              'SELECT id FROM acquittance_transactions WHERE pastorate_id = ? AND book_type = ? AND voucher_number = ? AND id != ?',
              ...
              (err, existingVoucher) => {
                ...
                this.db.db.run(
                  `UPDATE acquittance_transactions
  ...
  async deleteTransaction(transactionId, userId) {
    return new Promise((resolve) => {
      try {
        this.db.db.run(
          'DELETE FROM acquittance_transactions WHERE id = ?',
          [transactionId],
          function(err) {
            if (err) {
  ...
  async getNextVoucherNumber(pastorateId, bookType) {
    return new Promise((resolve) => {
      try {
        this.db.db.get(
          'SELECT MAX(voucher_number) as max_number FROM acquittance_transactions WHERE pastorate_id = ? AND book_type = ?',
        ...
        this.db.db.get(
          ...
          (err, totalResult) => {
            ...
            this.db.db.all(
              ...
              FROM acquittance_transactions aqt
              LEFT JOIN ledger_categories lc ON aqt.primary_category_id = lc.id
              WHERE aqt.pastorate_id = ? AND aqt.book_type = ?
...
module.exports = AcquittanceService;
